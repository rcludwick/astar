...
struct iax_session {
	...
	/* RFC 5456 §8.6 call-token state on outgoing NEW.
	 *   1. iax_call() sends NEW with empty CALLTOKEN IE (opt-in signal).
	 *   2. Server replies with IAX_COMMAND_CALLTOKEN carrying opaque bytes.
	 *   3. Dispatcher resends NEW with CALLTOKEN IE populated.
	 *   4. Normal AUTHREQ flow proceeds.
	 * A server with requirecalltoken=no replies to step 1 with AUTHREQ directly.
	 * Token lifetime is 10s server-side; we do not cache across calls. */
	unsigned char calltoken[256];
	int           calltoken_len;          /* 0 = not yet received */
	int           calltoken_resend;       /* 1 while iax_call() is replaying for resend */

	/* Cached iax_call() args so the dispatcher can replay on CALLTOKEN. */
	char ct_cidnum[80];
	char ct_cidname[80];
	char ct_lang[16];
	char ct_ich[256];
...
int iax_call(struct iax_session *session, const char *cidnum, const char *cidname, const char *ich, const char *lang, int wait, int formats, int capabilities)
{
	...
	/* RFC 5456 §8.6 — cache args for replay on CALLTOKEN reply, unless
	 * we are already inside a CALLTOKEN-triggered replay. */
	if (!session->calltoken_resend) {
		strncpy(session->ct_ich, ich, sizeof(session->ct_ich) - 1);
		session->ct_ich[sizeof(session->ct_ich) - 1] = '\0';
		if (cidnum) {
			strncpy(session->ct_cidnum, cidnum, sizeof(session->ct_cidnum) - 1);
			session->ct_cidnum[sizeof(session->ct_cidnum) - 1] = '\0';
		} else {
			session->ct_cidnum[0] = '\0';
		}
		if (cidname) {
			strncpy(session->ct_cidname, cidname, sizeof(session->ct_cidname) - 1);
			session->ct_cidname[sizeof(session->ct_cidname) - 1] = '\0';
		} else {
			session->ct_cidname[0] = '\0';
		}
		if (lang) {
			strncpy(session->ct_lang, lang, sizeof(session->ct_lang) - 1);
			session->ct_lang[sizeof(session->ct_lang) - 1] = '\0';
		} else {
			session->ct_lang[0] = '\0';
		}
		session->ct_formats = formats;
		session->ct_capabilities = capabilities;
		session->ct_have_cached = 1;
		/* Fresh call → no token yet. */
		session->calltoken_len = 0;
	}
	...
	/* Once-per-call session setup (skip on CALLTOKEN replay). */
	if (!session->calltoken_resend) {
	...
	/* RFC 5456 §8.6 — always append CALLTOKEN IE on outgoing NEW.
	 *   - calltoken_len == 0: empty payload signals "client supports tokens"
	 *   - calltoken_len  > 0: echo the opaque bytes the server sent us
	 * Servers with requirecalltoken=no ignore the empty IE and proceed. */
	if (session->calltoken_len > 0) {
		iax_ie_append_raw(&ied, IAX_IE_CALLTOKEN, session->calltoken, session->calltoken_len);
	} else {
	...
	if (e) {
		...
		case AST_FRAME_IAX:
			...
			case IAX_COMMAND_CALLTOKEN: {
				/* RFC 5456 §8.6 — server has provided a call token in
				 * response to our empty-IE NEW.  Stash the token and
				 * resend NEW with the populated IE.  Must happen inline
				 * — the server-side token has a 10s lifetime. */
				if (!e->ies.calltoken || e->ies.calltokendatalen <= 0) {
					DEBU(G "CALLTOKEN frame without CALLTOKEN IE; ignoring\n");
					free(e);
					e = NULL;
					break;
				}
				if (e->ies.calltokendatalen > (int)sizeof(session->calltoken)) {
					DEBU(G "CALLTOKEN: token too long (%d bytes); ignoring\n", e->ies.calltokendatalen);
					free(e);
					e = NULL;
					break;
				}
				if (!session->ct_have_cached) {
					DEBU(G "CALLTOKEN: no cached args to replay; ignoring\n");
					free(e);
					e = NULL;
					break;
				}
				...
				fprintf(stderr, "astar-iax2: CALLTOKEN received (%d bytes); resending NEW\n", e->ies.calltokendatalen);
				iax_call(session,
					session->ct_cidnum[0]  ? session->ct_cidnum  : NULL,
					session->ct_cidname[0] ? session->ct_cidname : NULL,
					session->ct_ich,
					session->ct_lang[0]    ? session->ct_lang    : NULL,
					0,
					session->ct_formats,
					session->ct_capabilities);
				session->calltoken_resend = 0;
				free(e);
				e = NULL;
				break;
			}
