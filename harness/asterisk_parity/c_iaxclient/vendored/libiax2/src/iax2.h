...
/* RFC 5456 §8.6 — anti-spoof token exchange, required by Asterisk 20+ (ASL3). */
#define IAX_COMMAND_CALLTOKEN  40      /* Call token response from server */
...
/* RFC 5456 §8.6 — call-token IE.  Empty payload from client = "I support
 * call tokens, please send me one"; populated payload (server→client or
 * echoed back client→server) carries the opaque token bytes. */
#define IAX_IE_CALLTOKEN                        54              /* Call token (opaque, variable length) */
