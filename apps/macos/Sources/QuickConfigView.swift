            ...
            return VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 8) {
                    ...
                    Picker(title, selection: selection) {
                        ...
                        // astar-9d41 — one row per DISTINCT name. Two devices
                        // can report the same one, and the engine addresses
                        // devices by name, so only the first is reachable; a
                        // second row would be a choice that silently opens the
                        // first device. It also duplicates the SwiftUI identity
                        // and tag, which is what made every matching row read
                        // as selected.
